      ...
      for (const [districtId, districtData] of Object.entries(stateData.districts)) {
        ...
        for (let i = 1; i <= PU_PER_DISTRICT; i++) {
          ...
          organizationsToInsert.push({
            ...
            orgn_id: nextOrgId(),
            orgn_type: "PU",
            orgn_name: `NSS Program Unit ${i} - ${districtName}, ${stateName}`,
          ...
          // e.g. pc.maharashtra.pune.1@gmail.com
          const email = `pc.${stateSlug}.${districtSlug}.${i}@gmail.com`;
          const mobile = String(mobileSeq++);

          enqueueUser(`PC ${districtName} Unit ${i}`, email, mobile, "Porgram_unit_coordinator", puOrgId);
          puCount++;
        }
      }
    }
